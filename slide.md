...
|3. **行為の詳細化**:開けるボタンを押す詳細な流れを組み立てる（何指で押す？）
|4. **行為の実行**:開けるボタンを押す
|5. **外界の状況の知覚** :ドアが開く
|6. **外界の状況の解釈** :  開いたと気付いた
|7. **結果の評価**:  ゴールを達成してよかった！
</div>
 <center>
...
<!-- 
補足しておきたい事項が2つある。
①行為の七段階は近似的モデルである。各段階ははっきりと分離したもの（この事項はこのフェーズに対応する）と厳密に言えない。
大抵の行動はすべてのフェーズを経由する必要はない。
②多くの活動は1つの行為で完成するものではないということ。ゴールをたとえば「たくさんの時間
・労力がかかる活動」としたときに、それに付随する「行為系列」は数多である。また、活動途中でゴールや意図が書き変わっていくこともある。る行動の結果が、ゴールを修正する材料になることがある、というのがいい例の1つとして挙げられます。
 -->
---
...
## へだたり（gulf）
- ほとんどの作業において、心の中にある意図・解釈・実際の行為・状況の間の関係がどのようになっているかを見つけ出すことは難しい
- この間の距離のことを「へだたり」と呼ぶ
- へだたりにより、心の中の状態と外界の状態が切り離される
<br>

- **へだたりがあること　→　ユーザにとっての重大な困難**

<!-- ほとんどの作業において、心の中にある意図・解釈・実際の行為・状況の間の関係がどのようになっているかを見つけ出すことは難しく、これらの間の距離のことを「へだたり」と呼びます。そして、へだたりにより、心の中の状態と外界の状態が切り離されるため、へだたりがあることはユーザーにとっての重大な困難となります。ここから、実行におけるへだたりと評価におけるへだたりに分けて説明します。 -->
---
...
- 実行におけるへだたりの大きさの尺度
  - ユーザが意図した通りの行為を直感的に行えるかどうか？
<!-- まず、実行におけるへだたりとは、ユーザの意図とそのシステムで許される行為の差異のことです。ユーザが意図した通りの行為をどれだけ直感的に行えるかが、実行におけるへだたりの大きさの一つの尺度となります。 -->
---
## 実行におけるへだたりの例（映写機）
...
<!-- 先ほどの映写機の例の場合、「映写機を準備してフィルムを上映する」という意図を達成するためにどんな行為をしなければならないかがまったく明らかでなく、実行におけるへだたりがありました。
- 一方、例えば、フィルムを自動巻き付けする映写機やVTR（カセットを機械に押し込むだけ再生可能）はテープを通す作業を機械に任せることができ、これらは、へだたりに橋をかけているといえます。
-->
...
- ユーザがシステムの物理的な状態を解釈したり、自分の予期や意図がどの程度よく満たされているかを判断する際の難易度
- 評価におけるへだたりの大きさの尺度
  - システムの状態がわかりやすく、解釈しやすいか？
  - ユーザの予測とシステムの反応がマッチしているか？

<!-- 次に、評価におけるへだたりとは、ユーザがシステムの物理的な状態を解釈したり、自分の予期や意図がどの程度よく満たされているかを判断する際の難易度のことです。評価のへだたりが小さいと、システムの状態がわかりやすく、また、解釈しやすく、システムの反応とユーザーの予測がマッチしているといえます。 -->

---

## 評価におけるへだたりの例（映写機）

- フィルムが映写機の中に挿入されているとき、本当に正しく巻き取られているかを判断するのが困難だった
- VTRの場合、カセットがちゃんと入っていない場合は、機械に収まらず、飛び出す
→うまくいかなかったことがわかる

<!-- これも、先ほどの映写機の例の場合、フィルムが映写機の中に挿入されているとき、本当に正しく巻き取られているかを判断するのが困難な状態でした。
一方、VTRはカセットがちゃんと入っていない場合は機械に収まらず、飛び出すのでうまくいかなかったことがわかります。
 -->
---

## デザインの手助けとしての行為の七段階理論

- 行為の七段階理論はデザインの貴重な手助けとなる
 ↓
- 評価と実行におけるへだたりに橋を架けることができているかどうかを確認するためのチェックリストとして利用可能
- これらは**よいデザインの原則**に相当する
  - 可視性
  - よい概念モデル
  - よい定義づけ
  - フィードバック

<!-- 行為の七段階の構造は、行為の七段階理論はデザインの貴重な手助けとなり、評価と実行におけるへだたりに橋を架けることができているかどうかを確認するためのチェックリストとして利用することが可能です（次のページに七段階理論（左）と、それを元にしたチェックリスト（右）を掲載）。これらは、本書の第１章で述べられていた「よいデザイン」の原則に相当し、七段階のうちの一つあるいはそれ以上の段階を支持するものとなっています。。 -->
---
